...
const ASSEMBLYAI_KEY = process.env.ASSEMBLYAI_API_KEY || 'b6d6101e7ded44a6921bc5a8146765a1';
...
const handler = async (req, res) => {
    ...
    if (!url) return res.status(400).json({ creator: 'Rikiz', status: false, message: 'Parameter url atau id diperlukan' });
    ...
    try {
        ...
        return res.status(500).json({ creator: 'Rikiz', status: false, message: err.message });
    }
};
