...
const handler = async (req, res) => {
    ...
    if (!url) return res.status(400).json({ creator: 'RIKI', status: false, message: 'Parameter url atau id diperlukan' });
    ...
    try {
        ...
        return res.status(500).json({ creator: 'RIKI', status: false, message: err.message });
    }
};
