var MP={
        ...
        <div id="mini-player" class="hidden fixed left-4 right-4 sm:left-auto sm:right-6 sm:w-96 z-[160]" style="bottom:82px;transition:transform 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275);will-change:transform;transform:translate3d(0,150px,0);">
            <div id="mini-player-inner" onclick="FullPlayer.open()" class="rounded-2xl px-3.5 py-2.5 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition-all relative overflow-hidden bg-white/[0.10] backdrop-blur-2xl border border-white/15 shadow-2xl group" style="box-shadow: 0 14px 34px rgba(0,0,0,0.55), 0 0 0 1px rgba(var(--accent-rgb),0.10);">
                ...
                <div class="flex items-center gap-1 z-10 shrink-0">
                    ...
                    <button id="mini-like-btn" onclick="toggleCurrentLike(); if(typeof event !== 'undefined') event.stopPropagation();" class="text-[#a0a5b0] hover:text-rose-400 active:scale-90 p-1 cursor-pointer" title="Sukai Lagu" aria-label="Sukai Lagu">
    ...
    applyColors(colors) {
        ...
        if (inner) inner.style.background = "linear-gradient(to right, rgba(var(--accent-rgb), 0.16), rgba(18, 19, 27, 0.92) 72%)";
