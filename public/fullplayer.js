var FullPlayer={
        ...
        <div id="full-player-card" class="fp-card flex flex-col justify-between p-3.5 pt-safe sm:p-6 sm:pt-safe">
            ...
            <div class="relative z-10 flex-shrink-0 w-full max-w-md mx-auto space-y-3 pb-2">
                ...
                <div class="flex items-center justify-between px-1 py-1">
                    <button id="full-shuffle-btn" onclick="SF()" class="relative text-white/50 hover:text-white active:scale-90 w-9 h-9 rounded-full flex items-center justify-center transition-colors cursor-pointer shrink-0" title="Acak (Shuffle)">
                        <i data-lucide="shuffle" class="w-4 h-4"></i>
                        <span id="full-shuffle-dot" class="hidden absolute top-1 right-1 w-1.5 h-1.5 bg-white rounded-full"></span>
                    </button>
                    <button id="full-prev-btn" onclick="PV()" class="text-white active:scale-90 w-14 h-14 rounded-full flex items-center justify-center transition-colors cursor-pointer shrink-0" title="Lagu Sebelumnya">
                        <i data-lucide="skip-back" class="w-8 h-8 fill-current"></i>
                    </button>
                    <button onclick="TP()" id="full-play-btn-wrap" class="relative text-[#050608] rounded-full hover:scale-105 active:scale-95 transition-all cursor-pointer flex items-center justify-center w-[76px] h-[76px] shrink-0" style="background:linear-gradient(135deg,var(--fp-accent, var(--accent)),var(--fp-accent-2, var(--accent-2))); box-shadow:0 10px 32px rgba(var(--fp-accent-rgb, var(--accent-rgb)),0.55);">
                        <div id="full-play-btn" class="flex items-center justify-center">
                            <i data-lucide="play" class="w-9 h-9 fill-current ml-0.5"></i>
                        </div>
                    </button>
                    <button id="full-next-btn" onclick="NX()" class="text-white active:scale-90 w-14 h-14 rounded-full flex items-center justify-center transition-colors cursor-pointer shrink-0" title="Lagu Berikutnya">
                        <i data-lucide="skip-forward" class="w-8 h-8 fill-current"></i>
                    </button>
                    <button onclick="TR()" id="btn-repeat" class="relative text-white/50 hover:text-white active:scale-90 w-9 h-9 rounded-full flex items-center justify-center transition-colors cursor-pointer shrink-0" title="Ulang (Repeat)">
    ...
    openAlbumInfo(){
        if(typeof showToast==='function') showToast('Informasi album tidak tersedia');
    },
    openOutputMenu(){
    ...
    openMoreSheet() {
        ...
        var rows = [
            { icon: 'download', label: 'Download', action: "gid('full-more-sheet') && FullPlayer.closeSheet('full-more-sheet');downloadCurrentSong();" },
            ...
            { icon: 'disc-3', label: 'Lihat Album', action: "FullPlayer.closeSheet('full-more-sheet');FullPlayer.openAlbumInfo();" },
        ...
        var extraRows = [
            ...
            { icon: 'sliders', label: 'Equalizer', action: "FullPlayer.closeSheet('full-more-sheet');openEqualizer();" },
