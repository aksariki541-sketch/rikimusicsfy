...
window.addEventListener('appinstalled',function(){
    ...
    showToast('MusifyRik berhasil diinstall!');
});
    ...
    modal.innerHTML = '<div class="bg-[#121318] border border-white/15 rounded-2xl p-5 max-w-xs w-full text-center space-y-3 shadow-2xl relative" onclick="event.stopPropagation()">'+
        ...
        '<div class="space-y-1">'+
            ...
            '<p class="text-white/60 text-xs leading-relaxed">'+
                'Fitur Mode Offline khusus untuk aplikasi PWA. Silakan install MusifyRik ke layar utama terlebih dahulu.'+
            '</p>'+
        '</div>'+
    ...
    if(deferredInstallPrompt){
        ...
        deferredInstallPrompt.userChoice.then(function(choice){
            if(choice.outcome==='accepted') {
                try { localStorage.setItem('pwa_installed', 'true'); } catch(e){}
                isStandaloneApp = true;
                showToast('Menginstall MusifyRik...');
            }
            deferredInstallPrompt=null;
            var btn=document.getElementById('pwa-install-btn');
            if(btn)btn.classList.add('hidden');
...
var App={
    init(){
        ...
        <div class="fixed bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-lg z-40">
            <div class="glass-dock rounded-[28px] py-1.5 px-1.5 flex items-center justify-between">
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='MusifyRik',cover=toHDCover('', videoId),artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'MusifyRik',cover:toHDCover('', videoId),artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();resetLyricsUI(videoId);
            FullPlayer.open();loadTrack(S.ct);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='MusifyRik',cover=toHDCover('', videoId);
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','MusifyRik',toHDCover('', videoId));});
    },
    ...
    switch(t){
        ...
        ['home','search','library','offline','liked','dev'].forEach(function(n){
            ...
            if(isCurrent){
                b.className = 'nav-item group relative flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation py-1.5 px-2.5 sm:px-3 rounded-2xl text-[#050608] font-bold transition-all duration-300 scale-105';
        ...
        popup.innerHTML = `
            <div class="glass-strong w-full max-w-sm rounded-3xl p-6 border border-white/10 text-center relative overflow-hidden" style="animation: slideUp 0.3s ease-out forwards;">
                <div class="relative w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center" style="background:linear-gradient(135deg,var(--accent),var(--accent-2));">
                    <i data-lucide="sparkles" class="w-8 h-8 text-[#050608]"></i>
                </div>
                <h2 class="text-2xl font-black chrome-text mb-1">MusifyRik 3.0</h2>
                <p class="text-white/70 text-xs mb-5">Upgrade besar: lebih stabil, lebih cepat, lebih rapi.</p>
                <div class="space-y-3 text-left mb-6 max-h-[260px] overflow-y-auto pr-1">
                    <div class="flex items-start gap-3"><div class="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center shrink-0 mt-0.5"><i data-lucide="list-music" class="w-4 h-4 text-white/80"></i></div><div><h4 class="text-white font-bold text-sm">Antrian & Queue Pintar</h4><p class="text-white/60 text-xs leading-relaxed">Putar berikutnya, tambah/hapus lagu, atur urutan, dan bersihkan antrian.</p></div></div>
                    <div class="flex items-start gap-3"><div class="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center shrink-0 mt-0.5"><i data-lucide="shuffle" class="w-4 h-4 text-white/80"></i></div><div><h4 class="text-white font-bold text-sm">Shuffle & Repeat 3 Mode</h4><p class="text-white/60 text-xs leading-relaxed">Shuffle tanpa pengulangan + mode Ulangi (Semua / Satu / Mati).</p></div></div>
                    <div class="flex items-start gap-3"><div class="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center shrink-0 mt-0.5"><i data-lucide="history" class="w-4 h-4 text-white/80"></i></div><div><h4 class="text-white font-bold text-sm">Terakhir Diputar</h4><p class="text-white/60 text-xs leading-relaxed">Lagu yang baru kamu dengar tersimpan otomatis di halaman utama.</p></div></div>
                    <div class="flex items-start gap-3"><div class="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center shrink-0 mt-0.5"><i data-lucide="more-vertical" class="w-4 h-4 text-white/80"></i></div><div><h4 class="text-white font-bold text-sm">Menu Tiga Titik</h4><p class="text-white/60 text-xs leading-relaxed">Tambah ke antrian/playlist, sukai, bagikan, dan info lagu langsung dari hasil pencarian.</p></div></div>
                    <div class="flex items-start gap-3"><div class="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center shrink-0 mt-0.5"><i data-lucide="gauge" class="w-4 h-4 text-white/80"></i></div><div><h4 class="text-white font-bold text-sm">Pencarian & Performa</h4><p class="text-white/60 text-xs leading-relaxed">Pencarian lebih responsif (debounce), loading/error/empty state, dan banyak perbaikan bug.</p></div></div>
                </div>
