...
app.use((req, res) => {
    ...
    if (req.path.startsWith('/play/')) {
        ...
        if (videoId) {
            ...
            const playTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist}` : qTitle) : `Dengarkan Musik - MusifyRik`;
            const playDesc = `Dengarkan ${qTitle || 'lagu favoritmu'} di MusifyRik Web Music Player`;
    ...
    if (req.path.startsWith('/artist/')) {
        ...
        if (artistId) {
            ...
            const pageTitle = qName ? `${qName} (Artist) - MusifyRik` : `Artist - MusifyRik`;
            const pageDesc = qName ? `Dengarkan lagu & album terbaik dari ${qName} di MusifyRik` : `Dengarkan lagu & album dari artist favoritmu di MusifyRik`;
    ...
    if (req.path.startsWith('/album/')) {
        ...
        if (albumId) {
            ...
            const pageTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist} (Album) - MusifyRik` : `${qTitle} (Album) - MusifyRik`) : `Album - MusifyRik`;
            const pageDesc = qTitle ? `Dengarkan album ${qTitle} di MusifyRik` : `Dengarkan album favoritmu di MusifyRik`;
            const coverUrl = qCover || `https://www.gobox.my.id/file/R0ym4wqfznmp.png`;
